...
%% Read from file
ImagesPath = 'F:\偏振采集系统\仿真\偏振\StokesParam\素材\偏振\线性度测试\croped\';
...
ImagesPath1 = '';
ImagesPath1 = strcat( ImagesPath, num2str( 60 + loop_number - 1 ) )  ;
...
% 线偏振部分x轴、y轴分量
param_linear_x = ( sqrt(Ix_polaried ) ) .* ( Ia_polaried - Ib_polaried)./(0.0001+abs(Ia_polaried - Ib_polaried) ) ;
% param_linear_x = ((Ix_polaried - stocks_s3) ) ;
param_linear_y =  sqrt(Iy_polaried )  ;
% param_linear_y =  (Iy_polaried - stocks_s3)  ;

%   param_Azimuth =  atan( param_linear_y ./ (param_linear_x + 0.0000001) )  ;
  param_Azimuth =  acos( param_linear_x ./ sqrt(param_linear_x .* param_linear_x + param_linear_y .* param_linear_y + 0.0000001) )  ;
%   param_Azimuth = param_Azimuth + pi*(1-( param_Azimuth)./(0.0001+abs(param_Azimuth) )/2 );
%  param_Azimuth = atan( param_linear_y ./ (param_linear_x + 0.0001) );
